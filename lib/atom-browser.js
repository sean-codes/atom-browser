...
export default {
  activate(state) {
    ...
    this.view = new AtomBrowserView(state);
  ...
  serialize() {
    return {
      deserializer: 'AtomBrowserView',
      url: this.view.html.webview.src
    };
  },
  ...
  listen() {
    ...
    this.view.html.addressbar.addEventListener('keyup', e => {
      if (e.key === 'Enter') this.setURL(this.view.html.addressbar.value);
    });
    ...
    this.view.html.btn.devtools.addEventListener('click', () =>
      this.devtools()
    );
    this.view.html.btn.livereload.addEventListener('click', () => {
      const active = !this.view.html.btn.livereload.classList.contains(
        'active'
      );
  ...
  back() {
    if (
      this.view.html.webview.getWebContents() &&
      this.view.html.webview.canGoBack()
    )
