...
export default class AtomBrowserViewBrowser {
   ...
   setListeners() {
      ...
      this.html.webview.addEventListener('console-message', ({ level, message, line, sourceId }) => {
         const levelFunction = ['log', 'info', 'warn', 'error'][level] || 'log'

         try {
            var path = new URL(sourceId)
         } catch(e) {
            var path = sourceId
         }

         console[levelFunction](`${message} [${path.pathname}:${line}]`)
      })
